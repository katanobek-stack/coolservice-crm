...
`measuredAt >= 2026-09-19T00:00:00Z` и `< 2026-09-20T00:00:00Z`: минимум
2026-09-19T00:34:26Z, максимум 2026-09-19T23:59:51Z. Распределение UTC:
00 — 3, 03 — 1, 04 — 2, 21 — 116, 22 — 118, 23 — 120; в остальных часах 0.

**Почему scanned больше planned.** Проверка безопасно читает все legacy
`packets`, потому что measurements лежат массивом внутри документов и Firestore
не умеет запросить вложенный `measurements[].measuredAt` с диапазоном. Это
ожидаемая логика legacy read-only dry-run, а не ошибка диапазона; фильтр
применяется к каждому measurement по `measuredAt`, не по `receivedAt`.
На момент аудита было 9 275 packet-документов (в предыдущем dry-run — 8 047):
контроллер продолжал естественно отправлять новые пакеты после первой проверки.

**Delayed.** В выбранном дне сейчас нет `deliveryQuality: delayed`, поэтому
доставок в другой UTC-день не найдено. Алгоритм всё равно включает delayed-точку,
если её `measuredAt` находится в окне: `receivedAt` применяется лишь к
unplaced-точкам, у которых `measuredAt` отсутствует.

**Без изменений.** Аудит был read-only: не запускались execute backfill,
checkpoint, deploy или изменения production-данных.
