...
import type { IntakeRepair, IntakeServiceType } from "../../shared/types/intake";
...
function IntakeVehicleModal({ intake, onClose }: { intake?: IntakeRepair; onClose: () => void }) {
  ...
  const [plate, setPlate] = useState(intake?.vehicle.plate ?? "");
  const [brand, setBrand] = useState(intake?.vehicle.brand ?? "");
  const [model, setModel] = useState(intake?.vehicle.model ?? "");
  const [serviceType, setServiceType] = useState<IntakeServiceType>(intake?.vehicle.serviceType ?? "refrigerator");
  const [photo, setPhoto] = useState(intake?.vehicle.photo ?? "");
  const [photoPath, setPhotoPath] = useState(intake?.vehicle.photoPath ?? "");
        ...
        <div className="flex items-center gap-3">
          ...
          <InlinePhotoButton
            ...
            onUploaded={async (photos) => {
              const p = photos[0];
              if (p) { setPhoto(p.url ?? ""); setPhotoPath(p.path ?? ""); }
            }}
          />
          {photo && (
            <button type="button" onClick={() => { setPhoto(""); setPhotoPath(""); }} style={{ fontSize: 12, color: "#dc2626", background: "transparent", border: "none", cursor: "pointer" }}>
              Убрать
            </button>
          )}
        </div>
...
function IntakeCard({ intake }: { intake: IntakeRepair }) {
  ...
  async function handleDelete() {
    if (!confirm(`Удалить заявку по машине ${vehicle.plate}? Все задачи и фото будут удалены.`)) return;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 14px 10px" }}>
        {vehicle.photo ? (
          <img src={vehicle.photo} alt="" style={{ width: 52, height: 52, borderRadius: 10, objectFit: "cover", flexShrink: 0, border: "1px solid var(--border)" }} />
        ) : (
          <div style={{ width: 52, height: 52, borderRadius: 10, flexShrink: 0, background: "rgba(59,130,246,0.15)", border: "1px solid rgba(59,130,246,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>🚚</div>
        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            <button type="button" onClick={() => setShowEdit(true)} title="Изменить машину" style={{ padding: "5px 7px", borderRadius: 8, background: "var(--bg3)", border: "1px solid var(--border)", color: "var(--text2)", cursor: "pointer", fontSize: 13, lineHeight: 1 }}>
      ...
      {showEdit && <IntakeVehicleModal intake={intake} onClose={() => setShowEdit(false)} />}
    </div>
  );
}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div className="crm-section" style={{ animation: "fadeUp 0.45s ease 0.1s both" }}>
        <div className="section-header">
          ...
          <span className="section-title">Машины на ремонте</span>
          {sorted.length > 0 && <span className="section-count">{sorted.length}</span>}
          <div className="section-actions">
            <button className="btn-primary" style={{ padding: "5px 12px", fontSize: 12 }} onClick={() => setShowNew(true)}>
              <i className="ti ti-plus" /> Новая машина
            </button>
          </div>
        </div>
        ...
        {sorted.length === 0 ? (
          <div style={{ padding: "36px 20px", textAlign: "center", color: "var(--text3)", fontSize: 13 }}>
            Нет машин в приёмке.<br />Нажмите «Новая машина», когда авто заедет на ремонт.
          </div>
        ) : (
      ...
      {showNew && <IntakeVehicleModal onClose={() => setShowNew(false)} />}
    </div>
  );
}
